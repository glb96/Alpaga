...
![Alpaga goal: a) robustly extract from raw spectra meaningful information from second harmonic generation experimental setups. (b) Sequence of second-harmonic signal processing steps: (1) suppression of spurious peaks and averaging of multiple acquisitions performed with the averaging_and_cleaning function; (2) noise estimation and correction across distinct processing regions using the remove_noise function; (3) extraction of the second-harmonic component followed by Gaussian fitting with the fit_gauss function. ](fig1.pdf){ width=85% }
